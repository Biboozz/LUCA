...
public class Image2 : MaskableGraphic {
    ...
    private void FillRegular(List<UIVertex> vbo) {
        ...
        float leftF = (left - pixelAdjustedRect.xMin) / pixelAdjustedRect.width;
        float rightF = (right - pixelAdjustedRect.xMin) / pixelAdjustedRect.width;
        float bottomF = (bottom - pixelAdjustedRect.yMin) / pixelAdjustedRect.height;
        float topF = (top - pixelAdjustedRect.yMin) / pixelAdjustedRect.height;
...
//        }
        ...
        if (growDirection == EnergyBarBase.GrowDirection.ExpandHorizontal) {
            float centerX = (rightF + leftF) / 2;

            corner1.x = Mathf.Lerp(centerX, leftF, fillValue);
            corner2.x = Mathf.Lerp(centerX, rightF, fillValue);
        } else {

            if (growDirection == EnergyBarBase.GrowDirection.RightToLeft) {
                corner1.x = Mathf.Lerp(rightF, leftF, fillValue);
            } else {
                corner1.x = 0f;
            }

            if (growDirection == EnergyBarBase.GrowDirection.LeftToRight) {
                corner2.x = Mathf.Lerp(leftF, rightF, fillValue);
            } else {
                corner2.x = rightF;
            }
        }
        ...
        if (growDirection == EnergyBarBase.GrowDirection.ExpandVertical) {
            float centerY = (topF + bottomF) / 2;

            corner2.y = Mathf.Lerp(centerY, topF, fillValue);
            corner1.y = Mathf.Lerp(centerY, bottomF, fillValue);
        } else {

            if (growDirection == EnergyBarBase.GrowDirection.BottomToTop) {
                corner2.y = Mathf.Lerp(bottomF, topF, fillValue);
            } else {
                corner2.y = 1f;
            }

            if (growDirection == EnergyBarBase.GrowDirection.TopToBottom) {
                corner1.y = Mathf.Lerp(topF, bottomF, fillValue);
            } else {
                corner1.y = 0f;
            }
        }
...
//        Vector2 corner2uv = new Vector2(sprite.rect.max.x / sprite.textureRect.width, sprite.rect.max.y / sprite.textureRect.height);
    ...
    private Vector4 FixUV(Vector4 outerUv) {
        // do not fix anything if Unity free
        ...
        // uv per pixel
        var drawingW = rect.width - padding.x - padding.z;
        var drawingH = rect.height - padding.y - padding.w;
        float pixelUvX = uvw / drawingW;
        float pixelUvY = uvh / drawingH;

        float leftAdd = padding.x;
        float rightAdd = padding.z;
        float bottomAdd = padding.y;
        float topAdd = padding.w;

        //Debug.Log("l: " + leftAdd + " b: " + bottomAdd + " r: " + rightAdd + " t: " + topAdd);

        float leftAddUV = pixelUvX * leftAdd;
        float rightAddUV = pixelUvX * rightAdd;
        float bottomAddUV = pixelUvY * bottomAdd;
        float topAddUV = pixelUvY * topAdd;

        //Debug.Log("2 l: " + leftAdd + " b: " + bottomAdd + " r: " + rightAdd + " t: " + topAdd);

        return new Vector4(outerUv.x - leftAddUV, outerUv.y - bottomAddUV, outerUv.z + rightAddUV, outerUv.w + topAddUV);
    ...
    private void FillQuad(List<UIVertex> vbo) {
        ...
        topRightQuad.Points(0.5f, 1, 1, 0.5f, uvs);
        AddToVbo(vbo, points, uvs);
        topRightQuad2.Points(sx2, sy, sx, sy2, points);
        topRightQuad2.Points(0.5f, 1, 1, 0.5f, uvs);
        AddToVbo(vbo, points, uvs);

        bottomRightQuad.Points(sx2, sy2, sx, 0, points);
        bottomRightQuad.Points(0.5f, 0.5f, 1, 0, uvs);
        AddToVbo(vbo, points, uvs);
        bottomRightQuad2.Points(sx2, sy2, sx, 0, points);
        bottomRightQuad2.Points(0.5f, 0.5f, 1, 0, uvs);
        AddToVbo(vbo, points, uvs);


        bottomLeftQuad.Points(0, sy2, sx2, 0, points);
        bottomLeftQuad.Points(0, 0.5f, 0.5f, 0, uvs);
        AddToVbo(vbo, points, uvs);
        bottomLeftQuad2.Points(0, sy2, sx2, 0, points);
        bottomLeftQuad2.Points(0, 0.5f, 0.5f, 0, uvs);
        AddToVbo(vbo, points, uvs);

        topLeftQuad.Points(0, sy, sx2, sy2, points);
        topLeftQuad.Points(0, 1, 0.5f, 0.5f, uvs);
        AddToVbo(vbo, points, uvs);
        topLeftQuad2.Points(0, sy, sx2, sy2, points);
        topLeftQuad2.Points(0, 1, 0.5f, 0.5f, uvs);
        AddToVbo(vbo, points, uvs);
    }
    ...
    private void AddToVbo(List<UIVertex> vbo, MadList<Vector2> points, MadList<Vector2> uvs) {
        ...
        for (int i = 0; i < points.Count; ++i) {
            ...
            vert.position = TransformPoint(point);
